    ...
    async create(ctx) {
        try {
            ...
            const lineItems = await Promise.all(products.map(async (product) => {
               // backend se data le kar aa rha
              const productEntities = await strapi.entityService.findMany("api::product.product", {
                  filters: {
                    //mujha wo key ki entities nikal ke do jo product key ke equal ho
                      key: product.key
                      //only unique key will found
                      // each product has unique key
                  }
              })
              //us product ke key ko realProduct me daal do
